"""Run the required image-classification study with simpler result outputs."""
...
from cnn2d_model import ConvNet2D
from image_data import DEFAULT_IMAGE_SIZE, ImageSplitDataset, prepared_image_paths
...
DEFAULT_IMAGE_SIZE_LOCAL = DEFAULT_IMAGE_SIZE
ARCHITECTURE_EPOCHS = 6
STUDY_EPOCHS = 4
...
def build_loaders(batch_size: int) -> dict[str, DataLoader]:
    paths = prepared_image_paths()
    datasets = {split_name: ImageSplitDataset(csv_path) for split_name, csv_path in paths.items()}
    return {
...
def train_run(
    ...
    model = ConvNet2D(
        input_size=DEFAULT_IMAGE_SIZE_LOCAL,
...
def save_test_predictions(destination: Path, predictions: list[int], labels: list[int]) -> None:
    ensure_directory(destination.parent)
    test_metadata = pd.read_csv(prepared_image_paths()["test"]).reset_index(drop=True)
    rows = []
...
def save_selected_examples(source_csv: Path, destination: Path, per_class: int = 10) -> None:
    ensure_directory(destination.parent)
    dataframe = pd.read_csv(source_csv)
    selected_parts = []
    for class_name in CLASS_NAMES:
        selected_parts.append(dataframe[dataframe["true_class"] == class_name].head(per_class))
...
def save_best_run_outputs(results_dir: Path, best_run: dict[str, object]) -> None:
    best_dir = ensure_directory(results_dir / "final_best_image_model")
    save_history_csv(best_dir / "epoch_metrics.csv", best_run["history"])
    plot_training_curves(
        best_dir / "training_curves.png",
        best_run["history"],
        title=f"Best image model: {best_run['run_name']}",
    )
...
def main() -> None:
    ...
    results_dir = ensure_directory(lab_root / "results" / "task3_images")
    ...
    for run_name, config in ARCHITECTURES.items():
        print(f"Training image architecture: {run_name}")
        result = train_run(
            run_name=run_name,
            model_config=config,
            loaders=loaders,
    ...
    plot_study_summary(results_dir / "architectures.png", "Image architectures", architecture_rows)
    ...
    for study_name, variants in STUDIES.items():
        ...
        for variant in variants:
            ...
            print(f"Training image study: {study_name} -> {run_name}")
            result = train_run(
                run_name=run_name,
                model_config=config,
                loaders=loaders,
        ...
        plot_study_summary(results_dir / f"{study_name}_summary.png", f"Image study: {study_name}", study_rows)
    ...
    print("Task 3 completed: image study finished.")
    print(f"Best image run: {best_with_test['run_name']}")
